...
export default {
  Query: {
    messages: async (_, { channelId }, context) => {
      const results = await context.prisma.channel
  ...
  Mutation: {
    createMessage: async (_, { channelId, recipient, text }, context) => {
      ...
      const results = await context.prisma.channel.findMany({
        where: {
          AND: [
      ...
      try {
        ...
        if (!channel) {
          throw new Error('La creation de la chaine de communication a echoue');
        }
